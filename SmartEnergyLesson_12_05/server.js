...
register.registerMetric(httpRequests); register.registerMetric(httpLatency); register.registerMetric(commandCounter); register.registerMetric(activeSessions);
...
function isValidText(v, min = 1, max = 255) { return typeof v === 'string' && v.trim().length >= min && v.trim().length <= max; }
...
async function fetchPrice() {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Elering status ${response.status}`);
    const data = await response.json();
    const ee = data?.data?.ee || [];
    const latest = ee[ee.length - 1];
    const price = latest?.price ? Number(latest.price) / 1000 : fallbackPrice();
    ...
    logger.warning('Elering API unavailable, fallback price used', { error: err.message, price });
...
async function applyAutomation(userId) {
  ...
  for (const d of devices) {
    ...
    if (newStatus !== d.status) {
      ...
      await notifyUser(user, `${d.name}: AUTO_${newStatus.toUpperCase()} при цене ${price}`);
    }
  }
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    if (!isValidText(username, 3, 100) || !isValidText(email, 5, 150) || !isValidText(password, 6, 100)) return res.status(400).json({ error: 'Логин от 3 символов, email и пароль от 6 символов обязательны' });
    const hash = await bcrypt.hash(password, 10);
    await pool.query('INSERT INTO users (username, email, password) VALUES (?, ?, ?)', [username.trim(), email.trim(), hash]);
    res.json({ success: true });
  } catch { res.status(400).json({ error: 'Пользователь или email уже существует' }); }
});
app.post('/api/login', async (req, res) => {
  const { username, password } = req.body;
  const [rows] = await pool.query('SELECT * FROM users WHERE username = ? AND active = TRUE', [username]);
...
app.post('/api/devices', requireAuth, async (req, res) => {
  ...
  await pool.query('INSERT INTO devices (user_id,name,description,price_limit,critical,connection_type,connection_value,connection_status,power_kw) VALUES (?,?,?,?,?,?,?,?,?)', [req.session.user.id, name.trim(), description || '', Number(price_limit), critical ? 1 : 0, connection_type || 'demo', connection_value || 'demo://device', status, Number(power_kw || 1)]);
...
app.use((err, req, res, next) => { logger.error('Unhandled error', { error: err.message, path: req.path }); res.status(500).json({ error: 'Серверная ошибка, но приложение не упало' }); });
app.listen(PORT, () => logger.info('Server started', { url: `http://localhost:${PORT}` }));
